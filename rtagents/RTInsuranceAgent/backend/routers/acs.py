...
async def acs_transcription_ws(ws: WebSocket):
    ...
    while True:
        ...
        try:
            ...
            logger.info("🎤📝 Transcription received : '%s' (status: %s, bot_speaking: %s)", 
                            text, status, bot_speaking)

            if status == "Intermediate" and bot_speaking:
                logger.info("🔊 Intermediate transcription received while bot is speaking, cancelling queue and ongoing media: '%s'", text)
