...
from rtagents.RTInsuranceAgent.backend.agents.tool_store.functions_helper import _json  # type: ignore
...
_REQUIRED_SLOTS = {
    "caller_role", "date_of_loss", "narrative",
    "collision", "bodily_injury", "property_damage", "comprehensive_loss",
    "loss_location.street", "loss_location.city",
    "loss_location.state", "loss_location.zipcode"
}
...
async def record_fnol(args: ClaimIntakeMinimal) -> Dict[str, Any]:
    ...
    claims_db.append({**args, "claim_id": claim_id, "status": "OPEN", "created_at": datetime.utcnow().isoformat()})
...
async def escalate_emergency(args: EscalateArgs) -> Dict[str, Any]:
    ...
    log.warning("🚨 Emergency escalation for %s (%s): %s",
                args["caller_name"], args["policy_id"], args["reason"])
