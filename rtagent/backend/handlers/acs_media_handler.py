...
from rtagent.backend.orchestration.orchestrator import route_turn
from rtagent.backend.services.acs.acs_helpers import broadcast_message
from rtagent.backend.shared_ws import send_response_to_acs
...
class ACSMediaHandler:
    ...
    def on_final(self, text, lang):
        ...
        if text and (
            "this call will join you to a conference" in text.lower()
            or "please say ok or press" in text.lower()
        ):
            ...
            try:
                ...
                logger.error(
                    f"❌ Failed to send DTMF tones for conference join: {e}",
                    exc_info=True,
                )
                return
            except asyncio.QueueFull:
                logger.warning(
                    "⚠️ Route turn queue is full, dropping conference response"
                )
                return
