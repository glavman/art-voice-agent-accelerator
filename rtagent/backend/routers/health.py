...
def _validate_phone_number(phone_number: str) -> tuple[bool, str]:
    ...
    if len(phone_number) < 8 or len(phone_number) > 16:  # Basic length validation
        return False, f"Phone number length invalid (8-15 digits expected): {phone_number}"
...
async def _check_acs_caller_fast(acs_caller) -> Dict:
    ...
    if not acs_caller:
        ...
        if not is_valid and ACS_SOURCE_PHONE_NUMBER and ACS_SOURCE_PHONE_NUMBER != "null":
            return {
