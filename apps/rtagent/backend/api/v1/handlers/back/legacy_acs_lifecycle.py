...
class ACSLifecycleHandler:
    ...
    async def start_outbound_call(
        ...
        orchestrator_name = getattr(active_orchestrator, 'name', 'unknown') if active_orchestrator else 'none'
        ...
        with tracer.start_as_current_span(
            ...
            try:
                logger.info(f"🚀 Starting outbound call to {target_number} with orchestrator: {orchestrator_name}")
        ...
        with tracer.start_as_current_span(
            ...
            try:
                for idx, event in enumerate(request_body):
                    ...
                    safe_set_span_attributes(span, {f"acs.event.{idx}.type": event_type})
                    ...
                    if event_type == "Microsoft.EventGrid.SubscriptionValidationEvent":
                        ...
                        if validation_code:
                            logger.info(f"✅ Event Grid subscription validation successful")
                            span.set_status(Status(StatusCode.OK))
                            return JSONResponse(
                        ...
                        logger.info(f"📞 Processing inbound call from {caller_id} with orchestrator: {orchestrator_name}")
                        ...
                        if answer_call_result:
                            ...
                            if call_connection_id:
                                ...
                                try:
                                    ...
                                    if hasattr(acs_caller, 'redis_mgr') and acs_caller.redis_mgr:
                                        cm.persist_to_redis(acs_caller.redis_mgr)
                                        
                                except Exception as e:
                                    logger.warning(f"Failed to initialize state: {e}")

                                logger.info(f"✅ Inbound call answered successfully: {call_connection_id}")
                            else:
                                logger.warning("Call answered but no call_connection_id available")
                    else:
                ...
                span.set_status(Status(StatusCode.ERROR, f"Unexpected error: {exc}"))
                logger.error(f"❌ Unexpected error during inbound call processing: {exc}")
    ...
    async def process_call_events(
        ...
        with tracer.start_as_current_span(
            ...
            logger.info(f"📋 Processing {len(events)} call events with orchestrator: {orchestrator_name}")
            ...
            try:
                ...
                span.set_status(Status(StatusCode.OK))
                logger.info(f"✅ Processed {result.get('processed_events', 0)} events successfully")
                ...
                span.set_status(Status(StatusCode.ERROR, f"Event processing error: {exc}"))
