...
class ThreadBridge:
    ...
    def schedule_barge_in(self, handler_func: Callable):
        ...
        logger.debug(f"🔍 Main loop state: {self.main_loop is not None and not self.main_loop.is_closed()}")
        ...
        try:
            ...
            logger.info("🚀 Barge-in scheduled via run_coroutine_threadsafe successfully")
            ...
            import traceback
            logger.error(f"❌ Schedule barge-in error traceback: {traceback.format_exc()}")
    ...
    def queue_speech_result(self, speech_queue: asyncio.Queue, event: SpeechEvent):
        ...
        try:
            ...
            logger.info(f"📋 Speech event queued via put_nowait: {event.event_type.value} - '{event.text}' (queue size: {speech_queue.qsize()})")
            return
        except asyncio.QueueFull:
            logger.warning(f"⚠️ Speech queue is full ({speech_queue.qsize()} items), trying run_coroutine_threadsafe approach...")
        except Exception as e:
            logger.debug(f"put_nowait failed: {type(e).__name__}: {e}, trying run_coroutine_threadsafe approach...")
        ...
        try:
            ...
            future = asyncio.run_coroutine_threadsafe(
                speech_queue.put(event), 
                self.main_loop
            )
            logger.info(f"📋 Speech event queued via run_coroutine_threadsafe: {event.event_type.value} - '{event.text}'")
            # Wait for the operation to complete to catch any errors
            future.result(timeout=0.1)
            logger.debug(f"📋 Speech event queue operation completed successfully via run_coroutine_threadsafe")
        except Exception as e:
            # Log the actual error details
            logger.error(f"❌ Failed to queue speech result via run_coroutine_threadsafe: {type(e).__name__}: {e}")
            logger.error(f"❌ Event details - Type: {event.event_type.value}, Text: '{event.text}', Queue size: {speech_queue.qsize()}")
...
class SpeechSDKThread:
    ...
    def _setup_callbacks(self):
        ...
        def on_partial(text: str, lang: str, speaker_id: Optional[str] = None):
            ...
            logger.info(f"🗣️ Partial speech detected in {lang}: '{text}' (length: {len(text)})")
            ...
            logger.debug(f"🔍 Thread bridge main loop available: {self.thread_bridge.main_loop is not None}")
        ...
        def on_error(error: str):
            ...
            error_event = SpeechEvent(
                event_type=SpeechEventType.ERROR,
                text=error
            )
        ...
        try:
            ...
            logger.debug(f"🔧 Callbacks set - partial: {on_partial}, final: {on_final}, error: {on_error}")
    ...
    def start_recognizer(self):
        ...
        try:
            ...
            logger.info(f"🔧 Recognizer config - languages: {getattr(self.recognizer, 'candidate_languages', 'unknown')}")
            logger.info(f"🔧 Recognizer config - audio format: {getattr(self.recognizer, 'audio_format', 'unknown')}")
            logger.info(f"🔧 Recognizer callbacks configured: partial={hasattr(self.recognizer, '_partial_callback')}, final={hasattr(self.recognizer, '_final_callback')}")
            
            # Start recognizer with safeguards against blocking
            logger.debug("🚀 Initiating speech recognizer start (should be non-blocking)")
            start_time = time.time()
            
            self.recognizer.start()
            
            start_duration = time.time() - start_time
            logger.info(f"✅ Speech recognizer started successfully (took {start_duration:.3f}s)")
            ...
            logger.debug(f"🔍 Recognizer write_bytes available: {hasattr(self.recognizer, 'write_bytes')}")
            ...
            try:
                ...
                logger.debug(f"🧪 Test audio sent successfully ({len(test_audio)} bytes in {test_duration:.3f}s)")
            ...
            asyncio.run_coroutine_threadsafe(
                self._test_barge_in_after_delay(),
            ...
            asyncio.run_coroutine_threadsafe(
                self._test_partial_callback_after_delay(),
                asyncio.get_event_loop()
            )
    ...
    async def _test_partial_callback_after_delay(self):
        ...
        logger.info("🧪 MANUAL PARTIAL CALLBACK TEST: Simulating partial speech detection...")
        try:
            ...
            import traceback
            logger.error(f"❌ Manual partial callback test traceback: {traceback.format_exc()}")
...
class RouteTurnThread:
    ...
    def __init__(
        ...
        self.current_response_task: Optional[asyncio.Task] = None  # Track current TTS/playback task
        self.running = False
        self._stopped = False  # Prevent multiple stop calls
    ...
    async def _processing_loop(self):
        ...
        with tracer.start_as_current_span(
            "v1.route_turn_thread.processing_loop",
            ...
            while self.running:
                try:
                    ...
                    logger.debug(f"🔄 Route Turn Thread waiting for events (queue size: {self.speech_queue.qsize()})")
                    speech_event = await asyncio.wait_for(
                        self.speech_queue.get(),
                        timeout=1.0  # Periodic check for shutdown
                    )
                    
                    logger.info(f"📢 Route Turn Thread received event: {speech_event.event_type.value} - '{speech_event.text}'")
                    ...
                    try:
                        if speech_event.event_type == SpeechEventType.FINAL:
                            ...
                            await self._process_direct_text_playback(speech_event, "greeting")
                        elif speech_event.event_type == SpeechEventType.ANNOUNCEMENT:
                            await self._process_direct_text_playback(speech_event, "announcement")
                        elif speech_event.event_type == SpeechEventType.STATUS_UPDATE:
                            await self._process_direct_text_playback(speech_event, "status_update")
                        elif speech_event.event_type == SpeechEventType.ERROR_MESSAGE:
                            await self._process_direct_text_playback(speech_event, "error_message")
                        elif speech_event.event_type == SpeechEventType.ERROR:
                            logger.error(f"Speech error in processing: {speech_event.text}")
                    except asyncio.CancelledError:
                        # Individual response task was cancelled (barge-in), but continue processing loop
                        logger.info(f"🛑 {speech_event.event_type.value} processing cancelled (barge-in), continuing loop")
                        continue
        ...
        with tracer.start_as_current_span(
            ...
            try:
                ...
                if self.orchestrator_func:
                    ...
                    logger.warning("⚠️ No orchestrator function provided, using fallback")
    ...
    async def _process_direct_text_playback(self, event: SpeechEvent, playback_type: str = "audio"):
        ...
        with tracer.start_as_current_span(
            ...
            try:
                logger.info(f"🎵 Playing {playback_type} through Route Turn Thread: {event.text}")
...
class MainEventLoop:
    ...
    def __init__(self, websocket: WebSocket, call_connection_id: str, route_turn_thread: Optional['RouteTurnThread'] = None):
        ...
        self.route_turn_thread = route_turn_thread  # Reference for barge-in cancellation
        ...
        try:
            ...
            if kind == "AudioMetadata":
                ...
                if not is_silent:
                    ...
                    if audio_bytes:
                        logger.debug(f"🎵 Non-silent audio received: {len(str(audio_bytes))} chars (base64)")
                        ...
                        if not recognizer:
                        ...
                        elif len(self.active_audio_tasks) >= self.max_concurrent_audio_tasks:
                            logger.warning(f"⚠️ Audio processing queue full ({len(self.active_audio_tasks)}/{self.max_concurrent_audio_tasks}), dropping frame")
                        else:
                            logger.debug(f"🎤 Recognizer ready, scheduling audio processing (queue: {len(self.active_audio_tasks)}/{self.max_concurrent_audio_tasks})...")
                            ...
                            logger.debug(f"🔄 Audio processing task created: {id(task)} (active: {len(self.active_audio_tasks)})")
                            
                            # Periodic queue health logging (every 5 seconds)
                            current_time = time.time()
                            if current_time - self.last_queue_health_log > 5.0:
                                queue_usage = len(self.active_audio_tasks) / self.max_concurrent_audio_tasks * 100
                                logger.info(f"📊 Audio queue health: {len(self.active_audio_tasks)}/{self.max_concurrent_audio_tasks} ({queue_usage:.1f}% utilization)")
            ...
            if total_duration > 0.005:  # Warn if total message processing takes more than 5ms
                logger.warning(f"⚠️ Message processing took {total_duration:.3f}s - should be < 5ms for non-blocking!")
    ...
    async def _process_audio_chunk_async(self, audio_bytes: Union[str, bytes], recognizer) -> None:
        ...
        try:
            ...
            if isinstance(audio_bytes, str):
                ...
                logger.debug(f"✅ Base64 decode completed in {decode_time:.3f}s: {len(audio_bytes)} bytes")
            ...
            try:
                ...
                logger.debug(f"✅ Audio chunk processed: {len(audio_bytes)} bytes in {total_time:.3f}s (executor: {executor_time:.3f}s)")
                ...
                logger.error(f"⏰ Audio processing timeout after {timeout_duration:.3f}s - recognizer.write_bytes hanging!")
                logger.error(f"⏰ This suggests the speech recognizer is blocked or unresponsive")
            ...
            logger.error(f"❌ Audio bytes type: {type(audio_bytes)}, length: {len(audio_bytes) if audio_bytes else 0}")
    ...
    async def _play_greeting_when_ready(self, acs_handler=None):
        ...
        logger.info(f"🎵 Queueing greeting for Route Turn Thread processing: {greeting_text}")
        ...
        try:
            ...
            acs_handler.thread_bridge.queue_speech_result(acs_handler.speech_queue, greeting_event)
            self.greeting_played = True
...
class ACSMediaHandler:
    ...
    async def start(self):
        ...
        with tracer.start_as_current_span(
            ...
            try:
                logger.info(f"🚀 Starting three-thread ACS media handler for call: {self.call_connection_id}")
                self.running = True
                
                # Capture the main event loop for cross-thread communication
                main_loop = asyncio.get_running_loop()
                self.thread_bridge.set_main_loop(main_loop)
                logger.info(f"🔗 Main event loop captured for cross-thread communication: {id(main_loop)}")
                ...
                logger.info("🎵 Greeting and recognizer scheduled for first AudioMetadata reception")
                
                span.set_status(Status(StatusCode.OK))  # Success status without description
    ...
    def queue_direct_text_playback(
        ...
        if playback_type not in direct_playback_types:
            logger.error(f"Invalid playback type: {playback_type}. Must be one of: {direct_playback_types}")
            return False
