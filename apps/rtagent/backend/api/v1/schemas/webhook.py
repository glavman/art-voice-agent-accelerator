...
class WebhookEvent(BaseModel):
    ...
    specversion: str = Field(default="1.0", description="CloudEvents specification version", json_schema_extra={"example": "1.0"})
    type: str = Field(
        ...,
    ...
    source: str = Field(
        ..., description="Event source identifier", json_schema_extra={"example": "/acs/calls/abc123"}
    )
    id: str = Field(..., description="Unique event identifier", json_schema_extra={"example": "event-123-abc"})
    time: Optional[str] = Field(None, description="Event timestamp in ISO 8601 format", json_schema_extra={"example": "2025-08-10T13:45:00Z"})
    ...
    data: Dict[str, Any] = Field(
        ...
        json_schema_extra={"example": {"callConnectionId": "abc123", "serverCallId": "server-abc123"}},
    )
...
class WebhookResponse(BaseModel):
    ...
    event_id: str = Field(
        ..., description="ID of the processed event", json_schema_extra={"example": "event-123-abc"}
    )
    processing_time_ms: Optional[int] = Field(
        None, description="Time taken to process the event in milliseconds", json_schema_extra={"example": 25}
    )
...
class WebhookBatchRequest(BaseModel):
    ...
    events: List[Union[WebhookEvent, ACSWebhookEvent, MediaWebhookEvent]] = Field(
        ..., description="List of webhook events to process", min_length=1, max_length=100
    )
    batch_id: Optional[str] = Field(None, description="Optional batch identifier for tracking", json_schema_extra={"example": "batch-123"})
...
class WebhookBatchResponse(BaseModel):
    ...
    batch_id: Optional[str] = Field(None, description="Batch identifier", json_schema_extra={"example": "batch-123"})
    total_events: int = Field(..., description="Total number of events in batch", json_schema_extra={"example": 5})
    successful_events: int = Field(..., description="Number of successfully processed events", json_schema_extra={"example": 4})
    failed_events: int = Field(..., description="Number of failed events", json_schema_extra={"example": 1})
    processing_time_ms: int = Field(..., description="Total processing time in milliseconds", json_schema_extra={"example": 150})
