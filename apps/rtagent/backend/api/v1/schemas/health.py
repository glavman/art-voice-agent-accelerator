...
class HealthResponse(BaseModel):
    ...
    details: Dict[str, Any] = Field(
        ...
        json_schema_extra={"example": {"api_version": "v1", "service": "rtagent-backend"}},
    )
...
class ServiceCheck(BaseModel):
    ...
    component: str = Field(..., description="Name of the component being checked", json_schema_extra={"example": "redis"})
    status: str = Field(
        ...,
        description="Health status of the component",
        json_schema_extra={"example": "healthy", "enum": ["healthy", "unhealthy", "degraded"]},
    )
...
class ReadinessResponse(BaseModel):
    ...
    status: str = Field(..., description="Overall readiness status", json_schema_extra={"example": "ready", "enum": ["ready", "not_ready", "degraded"]})
    timestamp: float = Field(
        ..., description="Timestamp when check was performed", example=1691668800.0
    )
    ...
    event_system: Optional[Dict[str, Any]] = Field(
        ...
        json_schema_extra={"example": {"is_healthy": True, "handlers_count": 7, "domains_count": 2}},
    )
    model_config = ConfigDict(json_schema_extra={
        "example": {
            "status": "ready",
