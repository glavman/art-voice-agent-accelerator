...
def build_and_flush(cm: ConversationManager, cosmos: CosmosDBMongoCoreManager):
    """
    ...
    try:
        ...
        logger.error(f"Failed to upsert analytics document for session {session_id}: {e}", exc_info=True)
