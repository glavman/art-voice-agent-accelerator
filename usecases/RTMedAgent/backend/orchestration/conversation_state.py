...
from usecases.RTMedAgent.backend.agents.prompt_store.prompt_manager import PromptManager
...
class ConversationManager:
    ...
    def ensure_system_prompt(self) -> None:
        if not any(m["role"] == "system" for m in self.hist):
            self.hist.insert(0, {"role": "system", "content": self._generate_system_prompt()})
