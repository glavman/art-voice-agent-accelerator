...
class DedicatedTtsPoolManager:
    ...
    async def get_dedicated_client(self, session_id: str) -> Tuple[SpeechSynthesizer, ClientTier]:
        """
        ...
        async with self._allocation_lock:
            ...
            raise RuntimeError(f"TTS pool exhausted, cannot allocate client for session {session_id}")
    ...
    async def _prewarm_pool_initial(self) -> None:
        ...
        logger.info(f"✅ Pre-warming complete: {warm_count}/{self._warm_pool_size} clients ready")
    ...
    async def _prewarming_loop(self) -> None:
        ...
        while not self._is_shutting_down:
            try:
                ...
                if deficit > 0:
                    logger.debug(f"🔥 Replenishing warm pool: {current_size}/{target_size} (+{deficit})")
