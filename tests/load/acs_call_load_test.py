...
class AcsCallLoadTester:
    ...
    async def initiate_single_call(self, target_number: str, call_id: str) -> AcsCallMetrics:
        ...
        try:
            ...
            metrics.call_established_time_ms = (call_established_time - start_time) * 1000
    ...
    async def run_load_test(self) -> Dict[str, Any]:
        ...
        async def run_single_call(call_index: int) -> AcsCallMetrics:
            async with semaphore:
                target_number = self.config.target_phone_numbers[call_index % len(self.config.target_phone_numbers)]
        ...
        if successful_calls:
            ...
            print(f"   P95:  {statistics.quantiles(acs_latencies, n=20)[18] if len(acs_latencies) >= 20 else max(acs_latencies):.1f}ms")
            ...
            print(f"   P95:  {statistics.quantiles(establishment_times, n=20)[18] if len(establishment_times) >= 20 else max(establishment_times):.1f}ms")
        ...
        return {
            ...
            "success_rate_percent": len(successful_calls)/len(valid_results)*100 if valid_results else 0,
            "acs_api_latency_stats": {
                "mean_ms": statistics.mean(acs_latencies) if acs_latencies else 0,
                "p95_ms": statistics.quantiles(acs_latencies, n=20)[18] if len(acs_latencies) >= 20 else max(acs_latencies) if acs_latencies else 0,
                "max_ms": max(acs_latencies) if acs_latencies else 0
            },
            "call_establishment_stats": {
                "mean_ms": statistics.mean(establishment_times) if establishment_times else 0,
                "p95_ms": statistics.quantiles(establishment_times, n=20)[18] if len(establishment_times) >= 20 else max(establishment_times) if establishment_times else 0,
...
async def main():
    ...
    parser.add_argument("--total-calls", type=int, default=1, help="Total number of calls to make")
    parser.add_argument("--concurrent", type=int, default=1, help="Number of concurrent calls")
    parser.add_argument("--duration", type=int, default=15, help="Call duration in seconds")
    ...
    config = AcsLoadTestConfig(
        ...
        callback_url=os.getenv("ACS_CALLBACK_URL", "https://your-domain.com/api/v1/acs/events")
    )
    
    # Validate that we have target phone numbers
    if not config.target_phone_numbers:
        print("❌ No target phone numbers provided!")
        print("   Either use --target-phones parameter or set ACS_TARGET_PHONE_NUMBER environment variable")
        print("   Examples:")
        print("     python tests/load/acs_call_load_test.py --target-phones +8165019907")
    ...
    print(f"   python tests/load/acs_call_load_test.py --target-phones {' '.join(config.target_phone_numbers)} --total-calls 5 --concurrent 2")
    print(f"   python tests/load/acs_call_load_test.py --target-phones {' '.join(config.target_phone_numbers)} --total-calls 10 --concurrent 3 --duration 30")
...
if __name__ == "__main__":
    ...
    if missing_vars:
        ...
        print("   export ACS_SOURCE_PHONE_NUMBER='+1234567890'     # Your ACS phone number")
        print("   export ACS_CONNECTION_STRING='endpoint=https://...'  # Your ACS connection string")
        print("   export ACS_TARGET_PHONE_NUMBER='+8165019907'     # Target phone to call (optional)")
        exit(1)
