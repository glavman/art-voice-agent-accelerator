...
class LoadTestResults:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        success_rate = (self.total_conversations_completed / max(1, self.total_conversations_attempted)) * 100
        ...
        if self.connection_times_ms:
            summary["connection_times_ms"] = {
                ...
                "p95": statistics.quantiles(self.connection_times_ms, n=20)[18] if len(self.connection_times_ms) >= 20 else max(self.connection_times_ms),
            }
        ...
        if self.conversation_durations_s:
            summary["conversation_durations_s"] = {
                ...
                "p95": statistics.quantiles(self.conversation_durations_s, n=20)[18] if len(self.conversation_durations_s) >= 20 else max(self.conversation_durations_s),
            }
        ...
        if self.agent_response_times_ms:
            summary["agent_response_times_ms"] = {
                ...
                "p95": statistics.quantiles(self.agent_response_times_ms, n=20)[18] if len(self.agent_response_times_ms) >= 20 else max(self.agent_response_times_ms),
            }
...
class ConversationLoadTester:
        ...
        async with semaphore:
            ...
            try:
                ...
                if metrics.total_agent_processing_time_ms > 0:
                    avg_agent_time = metrics.total_agent_processing_time_ms / max(1, metrics.user_turns)
                    self.results.agent_response_times_ms.append(avg_agent_time)
                
                if metrics.total_speech_recognition_time_ms > 0:
                    avg_speech_time = metrics.total_speech_recognition_time_ms / max(1, metrics.user_turns)
    ...
    async def run_load_test(self) -> LoadTestResults:
        ...
        print(f"📊 Config: {self.config.max_concurrent_conversations} max concurrent, {self.config.total_conversations} total")
        print(f"⏰ Duration: {self.config.test_duration_s}s with {self.config.ramp_up_time_s}s ramp-up")
        ...
        try:
            ...
            while time.time() < test_end_time and conversation_counter < self.config.total_conversations:
                ...
                if elapsed < self.config.ramp_up_time_s:
                    ...
                    target_concurrency = int(
                        (elapsed / self.config.ramp_up_time_s) * self.config.max_concurrent_conversations
                    )
                ...
                while (current_active < target_concurrency and 
                       conversation_counter < self.config.total_conversations and
                ...
                print(f"⏱️  Active: {len(active_tasks)}, Completed: {self.results.total_conversations_completed}, "
    ...
    def save_results(self, results: LoadTestResults, filename: Optional[str] = None) -> str:
    ...
    def print_summary(self, results: LoadTestResults):
        ...
        print(f"   Conversations: {summary['conversations_completed']}/{summary['conversations_attempted']}")
...
async def main():
    ...
    config = LoadTestConfig(
        ...
        conversation_templates=["insurance_inquiry", "quick_question", "confused_customer"],
    )
