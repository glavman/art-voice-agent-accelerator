...
class MultiTurnLoadTest:
    ...
    async def run_multi_turn_test(self, max_turns: int = 5) -> dict:
        ...
        config = LoadTestConfig(
            ...
            conversation_templates=["insurance_inquiry", "confused_customer", "claim_filing"],
            ws_url=self.base_url,
            output_dir="tests/load/results",
    ...
    async def run_escalating_turn_test(self, max_turns: int = 10) -> dict:
        ...
        config = LoadTestConfig(
            ...
            conversation_templates=["insurance_inquiry", "claim_filing", "policy_update"],
            ws_url=self.base_url,
            output_dir="tests/load/results",
            max_conversation_turns=max_turns,
            min_conversation_turns=1,
    ...
    def compare_turn_complexity_results(self, test_results: list) -> dict:
        ...
        for test_result in test_results:
            ...
            comparison["tests"][test_type] = {
                ...
                "avg_conversation_duration_s": summary.get("conversation_durations_s", {}).get("avg", 0),
                "conversations_completed": summary.get("conversations_completed", 0),
                "error_count": summary.get("error_count", 0),
            }
        
        # Print comparison table
        print(f"{'Test Type':<20} {'Max Turns':<10} {'Success%':<8} {'Avg Duration(s)':<15} {'Avg Response(ms)':<15} {'Errors':<7}")
        print(f"-" * 85)
        
        for test_type, metrics in comparison["tests"].items():
            print(f"{test_type:<20} "
        ...
        durations = [m["avg_conversation_duration_s"] for m in comparison["tests"].values() if m["avg_conversation_duration_s"] > 0]
        
        if len(turn_counts) > 1:
            comparison["turn_analysis"] = {
                "turn_range": f"{min(turn_counts)} - {max(turn_counts)} turns",
                "success_rate_trend": "stable" if max(success_rates) - min(success_rates) < 15 else "degrading",
                "duration_scalability": "linear" if durations and max(durations) / min(durations) < 3.0 else "exponential",
...
async def main():
    ...
    parser.add_argument("--turn-counts", nargs="+", type=int,
                       default=[1, 3, 5],
    ...
    summary_file = results_dir / f"multi_turn_test_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
